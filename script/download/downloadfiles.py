...
def getUniprot_trembl():
    subprocess.check_output(['bash', '-c', "wget ftp://ftp.uniprot.org/pub/databases/uniprot/current_release/knowledgebase/complete/uniprot_trembl.dat.gz"])
    subprocess.check_output(['bash','-c', "mv uniprot_sprot.dat.gz gene2go/"])
    subprocess.check_output(['bash','-c', "gunzip uniprot_sprot.dat.gz | egrep \"^ID   | ^AC  | ^DP | ^//"])
